...
class FramesAHARDataset(Dataset):
    """
    Loads pre-processed frame image datasets (from preprocess_dataset.py).
    Each clip is a folder of PNGs — no video decoding at runtime.
    Dramatically faster than AHARDataset for training.

    Expected structure: dataset_dir/class/clip_stem_fps8/0000.png ...
    """
    ...
    def __init__(
        ...
        # Each sample is a clip folder
        self.samples: list[tuple[Path, int]] = []
        for cls in self.class_names:
            for clip_dir in sorted((self.dataset_dir / cls).iterdir()):
                if clip_dir.is_dir() and any(clip_dir.glob("*.png")):
                    self.samples.append((clip_dir, self.class_to_idx[cls]))

        print(f"✅ {len(self.samples)} clips | {self.num_classes} classes: {self.class_names}")

    def __len__(self) -> int:
        return len(self.samples)

    def __getitem__(self, index: int):
        clip_dir, label = self.samples[index]

        png_files = sorted(clip_dir.glob("*.png"))[:self.sequence_length]

        frames = []
        for png in png_files:
            img = torchvision.io.read_image(str(png)).float().div(255.0)  # (C, H, W)
            if img.shape[1:] != self.frame_size:
                img = F.interpolate(img.unsqueeze(0), size=self.frame_size, mode="bilinear", align_corners=False)[0]
            if self.transform:
        ...
        return torch.stack(frames), label  # (T, C, H, W)
